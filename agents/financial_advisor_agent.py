...
class FinancialAdvisorAgent:
    ...
    def df(self) -> pd.DataFrame:
        if self._df is None:
            self._df = pd.read_csv(settings.ADVISOR_DATA_PATH)
    ...
    def tool_monthly_comparison(self, user_id: str) -> dict[str, Any]:
        ...
        return {
            ...
            "trend": "📈 UP" if change_pct > settings.ADVISOR_TREND_THRESHOLD_PCT else ("📉 DOWN" if change_pct < -settings.ADVISOR_TREND_THRESHOLD_PCT else "➡️ STABLE"),
    ...
    def tool_credit_score_impact(self, user_id: str, target_category: str | None = None) -> dict[str, Any]:
        ...
        base_credit_limit = max(settings.CREDIT_BASE_LIMIT_MIN, avg_monthly_spend * settings.CREDIT_LIMIT_MULTIPLIER)
        utilization = avg_monthly_spend / base_credit_limit
        
        # Credit Score calculation
        base = settings.CREDIT_BASE_SCORE
        score_adj = round((pos_ratio * settings.CREDIT_POS_WEIGHT) - (neg_ratio * settings.CREDIT_NEG_WEIGHT))
        if utilization > settings.CREDIT_UTILIZATION_THRESHOLD:
            score_adj -= round((utilization - settings.CREDIT_UTILIZATION_THRESHOLD) * settings.CREDIT_UTILIZATION_WEIGHT)
        ...
        result: dict[str, Any] = {
            ...
            "user_id": user_id,
            "estimated_credit_score": min(settings.CREDIT_SCORE_MAX, max(settings.CREDIT_SCORE_MIN, estimated_score)),
    ...
    def chat(self, message: str, user_id: str) -> dict[str, Any]:
        ...
        # Subscriptions / savings target (skip when user asks specifically about price hikes)
        is_price_hike_query = any(k in msg for k in ["hike", "price"])
        if any(k in msg for k in ["cancel", "subscription", "save $", "saving", "cut", "reduce"]) and not is_price_hike_query:
    ...
    def _compose_reply(self, question: str, tool_results: list[dict]) -> str:
        ...
        if self.llm and hasattr(self.llm, "generate"):
            try:
                ...
                logger.warning(f"LLM Reply generation failed, using template: {e}")
        parts = []

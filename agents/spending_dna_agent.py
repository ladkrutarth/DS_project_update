...
class SpendingDNAAgent:
    ...
    def df(self) -> pd.DataFrame:
        if self._df is None:
            self._df = pd.read_csv(settings.DNA_DATA_PATH)
        return self._df
    ...
    def compare_session(self, user_id: str, session_overrides: dict | None = None) -> dict[str, Any]:
        ...
        trust_score = round(max(0.0, 1.0 - composite_deviation * settings.DNA_DEVIATION_SCALE), 4)
        ...
        return {
            ...
            "verdict":              (
                "✅ Session matches DNA — Trusted"    if trust_score >= settings.DNA_TRUST_THRESHOLD else
                "⚠️ Moderate deviation — Verify"     if trust_score >= settings.DNA_MODERATE_THRESHOLD else

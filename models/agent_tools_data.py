...
DATA_PATHS = {
    "features": settings.FRAUD_TRAIN_PATH,
    "fraud_scores": settings.FRAUD_SCORES_PATH,
    "auth_profiles": settings.AUTH_PROFILES_PATH,
...
def _load_fraud_model():
    ...
    model_path = settings.MODELS_DIR / "fraud_model_rf.joblib"
    enc_path = settings.MODELS_DIR / "encoders.joblib"
...
def score_transaction(category: str, amt: float, merchant: str, hour: int, day_of_week: int) -> float:
    ...
    if model is not None and encoders is not None:
        try:
            ...
            # Fallback to heuristic if model fails, but log it
            from utils.logger import logger
            logger.warning(f"ML Model scoring failed: {e}. Falling back to heuristic.")
    # Fast heuristic

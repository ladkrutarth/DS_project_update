...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        logger.error(f"❌ RAG Engine failed to load: {e}", exc_info=True)

    # 2. GuardAgent (MLX)
    try:
        from models.guard_agent_local import LocalGuardAgent
        _agent = LocalGuardAgent()
        logger.info("✅ GuardAgent (Security Analyst) loaded.")
    except Exception as e:
        logger.warning(f"ℹ️ GuardAgent initialization informative error: {e}")
...
async def global_exception_handler(request: Request, exc: Exception):
    ...
    and provide structured error responses to the frontend.
    """

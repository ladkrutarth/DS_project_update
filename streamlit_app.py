...
API_BASE_URL = settings.VERISCAN_API_URL
CHART_TEXT_COLOR = settings.CHART_TEXT_COLOR
...
<style>
    ...
    .stApp {{
        ...
        color: {settings.TEXT_COLOR_MAIN};
    }}
    ...
    .metric-card .value {{ font-size: 2.6rem; font-weight: 800; margin: 0.5rem 0; color: {settings.TEXT_COLOR_DARK}; }}
    ...
    div.stButton > button {{
        background: linear-gradient(135deg, {settings.PRIMARY_COLOR} 0%, {settings.SECONDARY_COLOR} 100%) !important;
    ...
    .rag-answer {{
        ...
        font-size: 1.1rem;
        line-height: 1.8;
        color: {settings.TEXT_COLOR_DARK};  /* Extreme High Contrast Navy */
        margin-top: 2rem;
...
@st.cache_data(ttl=600)
def load_cfpb_data():
    if settings.CFPB_DATA_PATH.exists():
        # Large scale analysis enabled
        return pd.read_csv(settings.CFPB_DATA_PATH, nrows=50000)
    return pd.DataFrame()
...
def render_sidebar():
    with st.sidebar:
        ...
        fraud_df = load_fraud_data(settings.ADVISOR_DATA_PATH, settings.ADVISOR_DATA_PATH.stat().st_mtime if settings.ADVISOR_DATA_PATH.exists() else 0)
...
def render_omni_tab():
    ...
    try:
        ...
        logger.error(f"Failed to load users for selector: {e}")
        all_users = ["USER_0001"]
